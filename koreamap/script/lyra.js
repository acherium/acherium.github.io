...
class Window {
    constructor(param = {}) {
        ...
        this.node = {
            "main": create("div", ".window .window-border"),
        ...
        this.option = {
            ...
            borderless: typeof param["borderless"] !== "undefined" ? param["borderless"] : false,
            blurclose: typeof param["blurclose"] !== "undefined" ? param["blurclose"] : false
        };
        this.InactiveDetector = new EventTarget();
        this.zindex = -1;
        this.closed = false;
        ...
        if (this.option.movable) {
            this.node["window-title"].onpointerdown = (e1) => {
                ...
                this.node["window-title"].onpointermove = (e2) => {
                    ...
                    this.rect.x = `${this.node["main"].getBoundingClientRect().left + e2.movementX}px`;
                    this.rect.y = `${this.node["main"].getBoundingClientRect().top + e2.movementY}px`;
                    this.setPos();
                };
        ...
        if (this.option.blurclose) {
            this.InactiveDetector.addEventListener("inactive", () => {
                this.close();
            });
        };
    ...
    init() {
        fetch(this.href).then((res) => {
            res.text().then((html) => {
                ...
                this.node["window-title-main"].innerText = `${title.innerText} (uid: ${this.uid})`;
    ...
    active() {
        if (document.querySelector(".window-active") && document.querySelector(".window-active") !== this.node["main"]) {
            Array.from(document.querySelectorAll(".window-active")).forEach((node) => {
                const event = new CustomEvent("inactive", {
                    detail: {
                        from: parseInt(this.node["main"].getAttribute("uid"))
                    }
                });
                const target = lyra.ondisplay.modal[node.getAttribute("uid")];
                if (target && !target.closed) target.InactiveDetector.dispatchEvent(event);
        ...
        const list = Object.values(lyra.ondisplay.modal);
        this.zindex = list.sort((a, b) => b.zindex - a.zindex)[0].zindex + 1;
...
function getPointerPosition(e) {
    lyra.pointer.x = e.clientX;
    lyra.pointer.y = e.clientY;

    return 0;
};

document.addEventListener("pointermove", getPointerPosition);